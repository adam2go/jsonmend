...
HAND["ai-formats"] = [
    ("think-tag", '<think>I should call the weather tool.</think>\n'
     '{"city": "Paris"}', D, {"city": "Paris"},
     "reasoning-model <think> blocks are junk around the payload", JM),
    ("think-tag-fenced", '<think>plan: emit JSON</think>\n```json\n[1, 2]\n```',
     D, [1, 2], "<think> block before a fenced payload", JM),
    ("hermes-tool-call", '<tool_call>\n{"name": "get_weather", "arguments": '
     '{"city": "Paris"}}\n</tool_call>', D,
     {"name": "get_weather", "arguments": {"city": "Paris"}},
     "Hermes/Qwen-style <tool_call> XML wrapper strips", JM),
    ("hermes-truncated", '<tool_call>{"name": "f", "arguments": {"x": 1', D,
     {"name": "f", "arguments": {"x": 1}},
     "<tool_call> wrapper + truncation compose", JM),
    ("llama-function-tag", '<function=search>{"query": "cats"}</function>',
     D, {"query": "cats"},
     "Llama-style <function=...> wrapper strips", JM),
    ("react-action-input", 'Thought: I need to search.\nAction: search\n'
     'Action Input: {"query": "weather"}', D, {"query": "weather"},
     "ReAct agent format: the Action Input object is the payload", JM),
    ("react-final-answer", 'Final Answer: {"result": 42}', D,
     {"result": 42}, "ReAct final answer line", JM),
    ("sse-events", 'data: {"a":1}\n\ndata: {"a":2}\n\n', D,
     [{"a": 1}, {"a": 2}],
     "SSE-framed events: each data: payload is a value", JM),
    ("parallel-tool-calls",
     '[{"name":"f1","arguments":"{\\"a\\":1}"},{"name":"f2"', D,
     [{"name": "f1", "arguments": '{"a":1}'}, {"name": "f2"}],
     "parallel tool calls truncated mid-list", JM),
    ("degenerate-nesting", '{"a": {"a": {"a": {"a":', D,
     {"a": {"a": {"a": {"a": None}}}},
     "looping-model degeneration still closes cleanly", JM),
    ("degenerate-repeat-keys", '{"x": 1, "x": 1, "x": 1,', D, {"x": 1},
     "repeated identical members collapse", JM),
    ("inline-code-ticks", '`{"a":1}`', D, {"a": 1},
     "single-backtick inline code strips", JM),
    ("double-encoded", '"{\\"a\\": 1}"', D, '{"a": 1}',
     "a JSON-encoded string stays a string (it is already valid JSON)",
     JM),
    ("numpy-repr", '{"v": array([1, 2])}', A,
     [{"v": [1, 2]}, {"v": "[1, 2]"}, {"v": "array([1, 2])"}],
     "library reprs: unwrap the payload or keep it textual", JM),
    ("percent-value", '{"score": 95%}', D, {"score": "95%"},
     "percent values are not JSON numbers: keep as string", JM),
    ("currency-value", '{"price": $12.50}', D, {"price": "$12.50"},
     "currency values keep as string", JM),
    ("url-query-params", '{"u": https://a.b/c?d=1&e=2}', D,
     {"u": "https://a.b/c?d=1&e=2"},
     "bare URLs with query strings survive", JM),
    ("fence-close-truncated", '```json\n{"a":1}\n``', D, {"a": 1},
     "fence whose closing ticks got cut", JM),
    ("zwj-emoji", '{"family": "👨‍👩‍👧‍👦"}', D,
     {"family": "👨‍👩‍👧‍👦"},
     "ZWJ emoji sequences survive byte-exact", JM),
    ("rtl-text", '{"name": "مرحبا بالعالم"}', D,
     {"name": "مرحبا بالعالم"}, "RTL content survives", JM),
    ("combining-marks", '{"text": "café"}', D, {"text": "café"},
     "combining marks are not normalized away", JM),
    ("markdown-table-prose", "| col | val |\n|---|---|\n| a | 1 |\n\n"
     '{"a": 1}', D, {"a": 1},
     "markdown table prose before the value is junk", JM),
    ("env-assignment", "RESULT={\"ok\": true}", D, {"ok": True},
     "shell-style assignment prefix is junk", JM),
    ("xml-wrapped-truncated", '<response>\n{"items": ["a", "b"\n</response>',
     A, [{"items": ["a", "b"]}, {"items": ["a", "b", "</response>"]}],
     "XML wrapper with a truncated payload inside", JM),
    ("two-tool-calls-tagged",
     '<tool_call>{"name": "a"}</tool_call>\n'
     '<tool_call>{"name": "b"}</tool_call>', D,
     [{"name": "a"}, {"name": "b"}],
     "multiple tagged tool calls become an array", JM),
]
